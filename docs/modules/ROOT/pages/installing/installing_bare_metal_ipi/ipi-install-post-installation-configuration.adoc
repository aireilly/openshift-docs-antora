:_content-type: ASSEMBLY
[id="ipi-install-post-installation-configuration"]
= Installer-provisioned post-installation configuration
include::ROOT:partial$common-attributes.adoc[]
:context: ipi-install-post-installation-configuration

toc::[]

After successfully deploying an installer-provisioned cluster, consider the following post-installation procedures.

include::ROOT:partial$ipi-install-configuring-ntp-for-disconnected-clusters.adoc[leveloffset=+1]

include::ROOT:partial$nw-enabling-a-provisioning-network-after-installation.adoc[leveloffset=+1]

include::ROOT:partial$nw-osp-configuring-external-load-balancer.adoc[leveloffset=+1]

include::ROOT:partial$ipi-manual-migration-to-customdeploy.adoc[leveloffset=+1]

[role="_additional-resources"]
[id="additional-resources_ipi-install-post-installation-configuration"]
== Additional resources
* xref:machine_management/modifying-machineset.adoc#modifying-machineset[Modifying a machine set].
