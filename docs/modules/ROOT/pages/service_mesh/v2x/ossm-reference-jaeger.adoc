:_content-type: ASSEMBLY
[id="jaeger-config-ref"]
= Jaeger configuration reference
include::ROOT:partial$common-attributes.adoc[]
:context: jaeger-config-reference

toc::[]

When the {SMProductShortName} Operator deploys the `ServiceMeshControlPlane` resource, it can also create the resources for distributed tracing. {SMProductShortName} uses Jaeger for distributed tracing.

include::ROOT:partial$ossm-enabling-jaeger.adoc[leveloffset=+1]

include::ROOT:partial$ossm-config-smcp-jaeger.adoc[leveloffset=+1]

include::ROOT:partial$ossm-deploying-jaeger.adoc[leveloffset=+1]

include::ROOT:partial$ossm-configuring-external-jaeger.adoc[leveloffset=+1]

include::ROOT:partial$distr-tracing-deployment-best-practices.adoc[leveloffset=+2]

ifdef::openshift-enterprise,openshift-dedicated[]
For information about configuring persistent storage, see xref:storage/understanding-persistent-storage.adoc[Understanding persistent storage] and the appropriate configuration topic for your chosen storage option.
endif::[]

include::ROOT:partial$distr-tracing-config-security-ossm.adoc[leveloffset=+2]

include::ROOT:partial$distr-tracing-config-security-ossm-web.adoc[leveloffset=+3]

include::ROOT:partial$distr-tracing-config-security-ossm-cli.adoc[leveloffset=+3]

include::ROOT:partial$distr-tracing-config-default.adoc[leveloffset=+2]

include::ROOT:partial$distr-tracing-config-jaeger-collector.adoc[leveloffset=+2]

include::ROOT:partial$distr-tracing-config-sampling.adoc[leveloffset=+2]

include::ROOT:partial$distr-tracing-config-storage.adoc[leveloffset=+2]

ifdef::openshift-enterprise,openshift-dedicated[]
For more information about configuring Elasticsearch with {product-title}, see xref:logging/config/cluster-logging-log-store.adoc[Configuring the log store] or xref:distr_tracing/distr_tracing_install/distr-tracing-deploying-jaeger.adoc[Configuring and deploying distributed tracing].

//TO DO For information about connecting to an external Elasticsearch instance, see xref:distr_tracing/distr_tracing_install/distr-tracing-deploying-jaeger.adoc#jaeger-config-external-es_jaeger-deploying[Connecting to an existing Elasticsearch instance].
endif::[]

include::ROOT:partial$distr-tracing-config-query.adoc[leveloffset=+2]

include::ROOT:partial$distr-tracing-config-ingester.adoc[leveloffset=+2]
