:_content-type: ASSEMBLY
[id="ossm-observability"]
= Metrics, logs, and traces
include::ROOT:partial$common-attributes.adoc[]
:context: observability

toc::[]

Once you have added your application to the mesh, you can observe the data flow through your application. If you do not have your own application installed, you can see how observability works in {SMProductName} by installing the xref:service_mesh/v2x/prepare-to-deploy-applications-ossm.adoc#ossm-tutorial-bookinfo-overview_ossm-create-mesh[Bookinfo sample application].

include::ROOT:partial$ossm-observability-addresses.adoc[leveloffset=+1]

include::ROOT:partial$ossm-kiali-accessing-console.adoc[leveloffset=+1]

include::ROOT:partial$ossm-observability-visual.adoc[leveloffset=+1]

include::ROOT:partial$ossm-kiali-viewing-logs.adoc[leveloffset=+2]

include::ROOT:partial$ossm-kiali-viewing-metrics.adoc[leveloffset=+2]

include::ROOT:partial$ossm-distr-tracing.adoc[leveloffset=+1]

include::ROOT:partial$ossm-config-external-jaeger.adoc[leveloffset=+2]

include::ROOT:partial$ossm-config-sampling.adoc[leveloffset=+2]

include::ROOT:partial$ossm-jaeger-accessing-console.adoc[leveloffset=+1]

ifdef::openshift-enterprise[]
For more information about configuring Jaeger, see the xref:distr_tracing/distr_tracing_install/distr-tracing-deploying-jaeger.adoc#distr-tracing-deploy-default_deploying-distributed-tracing-platform[distributed tracing documentation].
endif::[]

include::ROOT:partial$ossm-access-grafana.adoc[leveloffset=+1]

include::ROOT:partial$ossm-access-prometheus.adoc[leveloffset=+1]
