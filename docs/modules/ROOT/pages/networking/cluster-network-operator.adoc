:_content-type: ASSEMBLY
[id="cluster-network-operator"]
= Cluster Network Operator in {product-title}
include::ROOT:partial$common-attributes.adoc[]
:context: cluster-network-operator

toc::[]

The Cluster Network Operator (CNO) deploys and manages the cluster network components on an {product-title} cluster, including the Container Network Interface (CNI) default network provider plugin selected for the cluster during installation.

include::ROOT:partial$nw-cluster-network-operator.adoc[leveloffset=+1]

include::ROOT:partial$nw-cno-view.adoc[leveloffset=+1]

include::ROOT:partial$nw-cno-status.adoc[leveloffset=+1]

include::ROOT:partial$nw-cno-logs.adoc[leveloffset=+1]

include::ROOT:partial$nw-operator-cr.adoc[leveloffset=+1]

[role="_additional-resources"]
[id="cluster-network-operator-additional-resources"]
== Additional resources

* xref:rest_api/operator_apis/network-operator-openshift-io-v1.adoc#network-operator-openshift-io-v1[`Network` API in the `operator.openshift.io` API group]
