:_content-type: ASSEMBLY
[id="about-metallb"]
= About MetalLB and the MetalLB Operator
include::ROOT:partial$common-attributes.adoc[]
:context: about-metallb-and-metallb-operator

toc::[]

As a cluster administrator, you can add the MetalLB Operator to your cluster so that when a service of type `LoadBalancer` is added to the cluster, MetalLB can add an external IP address for the service.
The external IP address is added to the host network for your cluster.

You can configure MetalLB so that the IP address is advertised with layer 2 protocols.
With layer 2, MetalLB provides a fault-tolerant external IP address.

You can configure MetalLB so that the IP address is advertised with the BGP protocol.
With BGP, MetalLB provides fault-tolerance for the external IP address and load balancing.

MetalLB supports providing layer 2 for some IP addresses and BGP for other IP addresses.

// When to deploy MetalLB
include::ROOT:partial$nw-metallb-when-metallb.adoc[leveloffset=+1]

// MetalLB Operator custom resources
include::ROOT:partial$nw-metallb-operator-custom-resources.adoc[leveloffset=+1]

// MetalLB software components
include::ROOT:partial$nw-metallb-software-components.adoc[leveloffset=+1]

// Layer 2
include::ROOT:partial$nw-metallb-layer2.adoc[leveloffset=+1]

// BGP
include::ROOT:partial$nw-metallb-bgp.adoc[leveloffset=+1]

// External traffic policy, common to layer 2 and BGP
include::ROOT:partial$nw-metallb-extern-traffic-pol.adoc[leveloffset=+1]

[id="limitations-and-restrictions_{context}"]
== Limitations and restrictions

// Infra considerations
include::ROOT:partial$nw-metallb-infra-considerations.adoc[leveloffset=+2]

// Layer 2 limitations
include::ROOT:partial$nw-metallb-layer2-limitations.adoc[leveloffset=+2]

// BGP limitations
include::ROOT:partial$nw-metallb-bgp-limitations.adoc[leveloffset=+2]

// Incompat with IP failover
[id="incompatibility-with-ip-failover_{context}"]
=== Incompatibility with IP failover

MetalLB is incompatible with the IP failover feature. Before you install the MetalLB Operator, remove IP failover.

[role="_additional-resources"]
[id="additional-resources_about-metallb-and-metallb-operator"]
== Additional resources

* xref:networking/configuring_ingress_cluster_traffic/overview-traffic.adoc#overview-traffic-comparision_overview-traffic[Comparison: Fault tolerant access to external IP addresses]

* xref:networking/configuring-ipfailover.adoc#nw-ipfailover-remove_configuring-ipfailover[Removing IP failover]

